<template>
  <TableToolbar>
    ...
    <template #right>
      <ButtonCreate route="contractSpecification.create" />
    </template>
  </TableToolbar>
  ...
  <el-table v-loading="loading" :data="result.items" table-layout="auto">
    ...
    <el-table-column :label="$t('common.operations')">
      <template #default="{ row }">
        <TextLink
          :to="{ name: 'contractSpecification.edit', params: { id: row.id } }"
        />
      </template>
    </el-table-column>
  </el-table>
...
import contractSpecificationApi, {
  type ContractSpecificationRow,
  type GetContractSpecificationsQuery,
...
const query: GetContractSpecificationsQuery = reactive({
  ...defaultQuery,
});
const result: PageResult<ContractSpecificationRow> = reactive(defaultResult());
