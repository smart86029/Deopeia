...
import { contractApi, type Contract, type ContractLocale } from '@/api/trading/contract-api';
import { UnderlyingType } from '@/models/instrument-type';
...
const save = () => {
  loading.value = true;
  const post = props.action === 'create' ? contractApi.create : contractApi.update;
  post(form)
